...
def generate_css():
    ...
    for group_key, primary_color in group_colors.items():
        ...
        scss_content.append('    $theme-colors-bg-subtle: map-merge($theme-colors-bg-subtle, ($color: mix(#fff, $value, 80%)));')
        scss_content.append('  }')
        scss_content.append('  @if not map-has-key($theme-colors-border-subtle, $color) {')
        scss_content.append('    $theme-colors-border-subtle: map-merge($theme-colors-border-subtle, ($color: mix(#fff, $value, 60%)));')
        scss_content.append('  }')
